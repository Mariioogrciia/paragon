...
## Sesión del 22 de septiembre de 2026 — auditoría completa + arreglo del trabajo de Antigravity

Pedido: auditoría de rendimiento/estética de toda la plataforma y de la app
Android, y después arreglar lo que hiciera falta, en autónomo (el usuario
no iba a estar delante).

### Lo que había dejado Antigravity a medias (sin commitear, en el working tree)
Sistema de clanes (`/clanes`) y un radar de rivalidad (recharts) en el
comparador — **no compilaba**: columnas inventadas en `user_game`/`games`
(el desglose por metal vive en el jsonb `earned`, no en columnas sueltas),
propiedades que no existen en `ParagonProgress` (`.nivel`/`.xpTotal` en vez
de `.level`/`.xp`), y `trophyScore()` llamado con la forma de argumentos de
otra función. Tampoco tenía migración de sus propias tablas.
- `scripts/crear-tabla-clanes.mts` (**ya ejecutada contra producción**, con
  `DIRECT_URL` — con `DATABASE_URL` da timeout en DDL, motivo de siempre).
- `getClanScore` reescrito para reutilizar `getParagonScore`/`trophyScore`
  de verdad en una sola consulta batched (antes: una query por miembro
  dentro de un `for`, con una fórmula que no era la suya).
- Nombre/etiqueta/descripción del clan pasan por `errorSiOfensivo`, igual
  que el resto del texto libre visible a otros.
- Índice único en `clan_members.userId`: cierra la carrera de "unirse a dos
  clanes a la vez" a nivel de base de datos, y sirve de índice para
  `getUserClan` (se llama en cada visita a `/u/[handle]`).
- Avatar de miembros con `avatarUrlSql` en vez de `users.image` a pelo.
- `RivalryRadar` (recharts) con `next/dynamic`+`ssr:false`.
- "Clanes" enlazado de verdad en el menú "Más" del header (i18n en los 4
  idiomas) y en `NAV_OCULTABLE` — antes no había ninguna forma de llegar
  ahí desde la interfaz.

### Auditoría: alta prioridad, arreglado
- Landing (`/`) y panel: waterfalls de queries independientes fusionados en
  `Promise.all` (la landing es la página pública más visitada).
- `/u/[handle]`: tres `Promise.all` consecutivos e independientes,
  fusionados en uno.
- `TiltCard`: nuevo prop `innerClassName` — separa "quién recorta"
  (`overflow-hidden`) de "quién proyecta el resplandor de hover", mismo
  elemento no puede hacer las dos cosas sin recortarse a sí mismo (ver la
  trampa ya documentada arriba). Corregido en `LibraryGrid` (mosaico),
  `Planificador`, `ShowcaseTrophies` — de paso arregla el brillo de
  `holo-card`, que sufría lo mismo.
- `TarjetaNoticia`: mismo patrón, resuelto con un div interior.
- Contraste roto: `bg-accent`/`hover:bg-accent` con `text-white` (8 sitios)
  y una variable CSS inventada por Antigravity (`--accent-text-contrast`,
  no existe en `globals.css`) sustituidos por `text-background`, el patrón
  real que ya usa el resto de la app.
- `ComparePairGames`: `min-w-0` que faltaba en un título dentro de columna
  de grid `1fr`.
- `prefers-reduced-motion`: ninguna animación lo respetaba (marquee, brillo
  holográfico, marcos de avatar, glint, tilt 3D). Añadida la media query
  + clase compartida para los marcos (animan vía `style` inline).
- `next.config.ts`: `images.remotePatterns` para IGDB y Steam.

### Deliberadamente NO tocado hoy
- **Migrar `<img>` a `next/image` en las carátulas de la biblioteca**
  (37 de 38 usos son `<img>` crudo): PSN y Xbox dan URLs de imagen
  dinámicas desde `psn-api`/`xbl.io`, no fijadas en nuestro código —
  `next/image` da error duro (no una imagen rota) contra un host no
  declarado en `remotePatterns`. Sin una cuenta real para confirmar los
  hostnames en vivo y sin el usuario delante para revisar el resultado,
  el riesgo de dejar carátulas rotas en producción sin supervisión pesaba
  más que la ganancia de LCP. Siguiente paso: confirmar los hosts contra
  una cuenta real y entonces migrar.
- Objetivos táctiles bajo 44px, duplicación `CustomSelect`/`Dropdown`,
  `FocusMode` con colores fijos — señalados en la auditoría, de prioridad
  media/baja, no aplicados por tiempo.
- **Android**: sin `signingConfig` de release ni `minifyEnabled` — no
  urgente, `PLAN-NATIVA.md` confirma que publicar en Play Store está
  descartado a propósito por ahora.

Verificado contra el dev server real (no solo `tsc`/`eslint`): `/`,
`/clanes` (con las tablas nuevas ya creadas) y `/comparar/[handle]`
responden 200 sin error de servidor.
