...
const ACHIEVEMENTS = [
  { id: "first_blood", name: "Primera joya", description: "Consigue tu primer platino.", target: 1, value: (games: Game[]) => games.reduce((n, g) => n + (g.earned?.platinum ?? 0), 0), color: "var(--platinum)" },
  { id: "cazador", name: "Cazador", description: "Consigue 10 platinos.", target: 10, value: (games: Game[]) => games.reduce((n, g) => n + (g.earned?.platinum ?? 0), 0), color: "var(--accent)" },
  { id: "experto", name: "Experto", description: "Consigue 50 platinos.", target: 50, value: (games: Game[]) => games.reduce((n, g) => n + (g.earned?.platinum ?? 0), 0), color: "var(--gold)" },
  { id: "leyenda", name: "Leyenda", description: "Consigue 100 platinos.", target: 100, value: (games: Game[]) => games.reduce((n, g) => n + (g.earned?.platinum ?? 0), 0), color: "var(--bronze)" },
  ...
  return (
    <section className="rounded-[18px] border border-border bg-surface p-5">
      <div className="mb-4 flex flex-wrap items-end justify-between gap-2">
        ...
        <span className="text-xs font-semibold text-muted">{earnedIds.length}/{ACHIEVEMENTS.length} conseguidos</span>
      </div>
      <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {ACHIEVEMENTS.map((achievement) => {
          const value = achievement.value(games);
          const earned = earnedIds.includes(achievement.id) || value >= achievement.target;
