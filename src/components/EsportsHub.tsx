  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="text-sm font-bold text-muted mr-2">FILTRAR POR:</span>
        <button 
          onClick={() => setFilter("Todos")} 
          className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${filter === "Todos" ? "text-background" : "bg-surface border border-border text-muted hover:text-foreground hover:bg-surface-2"}`}
        ...
        {allGames.map(game => (
          <button 
            ...
            className={`px-4 py-2 rounded-full text-sm font-bold transition-colors ${filter === game ? "text-background" : "bg-surface border border-border text-muted hover:text-foreground hover:bg-surface-2"}`}
                ...
                filteredLive.map((match) => (
                  <div key={match.id} className="rounded-2xl border border-border bg-surface p-6 shadow-lg overflow-hidden relative group transition-all hover:border-[rgb(var(--accent-rgb)/0.5)]">
                    ...
                    <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-6">
                      <div className="flex flex-col items-center sm:items-start w-32">
                        <span className="text-[0.6875rem] font-bold text-muted uppercase tracking-wider mb-1 text-center sm:text-left">{match.game} • {match.league}</span>
                        <span className="text-xs font-semibold px-2 py-0.5 rounded bg-surface-2 border border-border">{match.format}</span>
                      </div>
                      
                      <div className="flex items-center gap-4 sm:gap-6 w-full sm:w-auto justify-center flex-1">
                        <div className="flex flex-col items-center gap-2 flex-1 sm:flex-none">
                          ...
                          <span className="font-bold text-xs sm:text-sm text-center line-clamp-1">{match.team1.name}</span>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-2 flex-1 sm:flex-none">
                          ...
                          <span className="font-bold text-xs sm:text-sm text-center line-clamp-1">{match.team2.name}</span>
                        </div>
                      </div>
                      
                      <div className="mt-4 sm:mt-0 flex flex-col items-center justify-center w-32">
                        {match.streamUrl ? (
                          <a 
                            href={match.streamUrl} 
                            target="_blank" 
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 bg-[#9146FF] hover:bg-[#a970ff] text-white px-4 py-2 rounded-xl font-bold text-xs transition-all shadow hover:-translate-y-0.5 w-full justify-center"
                          >
                            <SiTwitch size={14} />
                            Twitch
                          </a>
                        ) : (
                          <span className="text-xs text-muted font-medium bg-surface-2 px-3 py-1.5 rounded-lg border border-border">Sin stream</span>
                        )}
                      </div>
                    </div>
                  </div>
