  ...
  return (
    <section className="rounded-[18px] border border-border bg-surface p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <select value={sort} onChange={(event) => setSort(event.target.value as typeof sort)} className="rounded-lg border border-border bg-background px-3 py-2 text-xs font-semibold text-foreground"><option value="proximidad">Más cerca</option><option value="progreso">Más progreso</option><option value="horas">Más horas</option></select>
      </div>
